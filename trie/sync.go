...
type Sync struct {
	database czzdb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database czzdb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
// both czz/6x style fast sync and snap/1x style state sync, the paths of trie
...
func (s *Sync) Commit(dbw czzdb.Batch) error {
