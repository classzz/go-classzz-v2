...
// support all the operations needed by the Classzz chain protocols.
...
type handlerConfig struct {
	Database   czzdb.Database            // Database for direct sync insertions
...
// newHandler returns a handler for all Classzz chain management protocol.
...
func (h *handler) runEthPeer(peer *eth.Peer, handler eth.Handler) error {
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Classzz handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Classzz peer connected", "name", peer.Name())
	...
	if err := h.downloader.RegisterPeer(peer.ID(), peer.Version(), peer); err != nil {
		peer.Log().Error("Failed to register peer in czz syncer", "err", err)
		return err
	}
	if snap != nil {
...
// runSnapExtension registers a `snap` peer into the joint czz/snap peerset and
// starts handling inbound messages. As `snap` is only a satellite protocol to
// `czz`, all subsystem registrations and lifecycle management will be done by
// the main `czz` handler to prevent strange races.
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("Classzz peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `czz` peer if it exists
	logger.Debug("Removing Classzz peer", "snap", peer.snapExt != nil)
...
func (h *handler) Start(maxPeers int) {
	...
	h.wg.Add(2)
	go h.chainSync.loop()
	go h.txsyncLoop64() // TODO(karalabe): Legacy initial tx echange, drop with czz/64.
}
...
func (h *handler) Stop() {
	...
	log.Info("Classzz protocol stopped")
}
