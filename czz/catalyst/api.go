...
import (
	...
	chainParams "github.com/classzz/go-classzz-v2/params"
...
func newConsensusAPI(czz *czz.Classzz) *consensusAPI {
	return &consensusAPI{czz: czz}
}
...
func (api *consensusAPI) makeEnv(parent *types.Block, header *types.Header) (*blockExecutionEnv, error) {
	state, err := api.czz.BlockChain().StateAt(parent.Root())
	if err != nil {
		return nil, err
	}
	env := &blockExecutionEnv{
		chain:   api.czz.BlockChain(),
		state:   state,
		header:  header,
...
func (api *consensusAPI) AssembleBlock(params assembleBlockParams) (*executableData, error) {
	...
	coinbase, err := api.czz.Etherbase()
	if err != nil {
		return nil, err
	}
	num := parent.Number()
...
func (api *consensusAPI) NewBlock(params executableData) (*newBlockResponse, error) {
	parent := api.czz.BlockChain().GetBlockByHash(params.ParentHash)
	if parent == nil {
