...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Classzz
...
func (czz *Classzz) startBloomHandlers(sectionSize uint64) {
	for i := 0; i < bloomServiceThreads; i++ {
		go func() {
			for {
				select {
				case <-czz.closeBloomHandler:
					return

				case request := <-czz.bloomRequests:
					task := <-request
					task.Bitsets = make([][]byte, len(task.Sections))
					for i, section := range task.Sections {
						head := rawdb.ReadCanonicalHash(czz.chainDb, (section+1)*sectionSize-1)
						if compVector, err := rawdb.ReadBloomBits(czz.chainDb, task.Bit, section, head); err == nil {
