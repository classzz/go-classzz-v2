...
// parent block.
func (czz *Classzz) stateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, checkLive bool) (statedb *state.StateDB, err error) {
	var (
		current  *types.Block
	...
	if checkLive {
		statedb, err = czz.blockchain.StateAt(block.Root())
		if err == nil {
			return statedb, nil
		}
	}
	if base != nil {
		...
		current = czz.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	} else {
		...
		database = state.NewDatabaseWithConfig(czz.chainDb, &trie.Config{Cache: 16})
		...
		for i := uint64(0); i < reexec; i++ {
			...
			parent := czz.blockchain.GetBlock(current.ParentHash(), current.NumberU64()-1)
			if parent == nil {
	...
	for current.NumberU64() < origin {
		...
		if current = czz.blockchain.GetBlockByNumber(next); current == nil {
			return nil, fmt.Errorf("block #%d not found", next)
		}
		_, _, _, err := czz.blockchain.Processor().Process(current, statedb, vm.Config{})
		if err != nil {
...
func (czz *Classzz) stateAtTransaction(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, error) {
	// Short circuit if it's genesis block.
	if block.NumberU64() == 0 {
		return nil, vm.BlockContext{}, nil, errors.New("no transaction in genesis")
	}
	// Create the parent state database
	parent := czz.blockchain.GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.BlockContext{}, nil, fmt.Errorf("parent %#x not found", block.ParentHash())
	}
	// Lookup the statedb of parent block from the live database,
	// otherwise regenerate it on the flight.
	statedb, err := czz.stateAtBlock(parent, reexec, nil, true)
	if err != nil {
