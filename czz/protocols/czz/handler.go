...
import (
	...
	"github.com/classzz/go-classzz-v2/p2p/enr"
...
type Backend interface {
	...
	// RunPeer is invoked when a peer joins on the `czz` protocol. The handler
	// should do any peer maintenance work, handshakes and validations. If all
	// is passed, control should be given back to the `handler` to process the
	// inbound messages going forward.
	RunPeer(peer *Peer, handler Handler) error

	// PeerInfo retrieves all known `czz` information about a peer.
...
// MakeProtocols constructs the P2P protocol definitions for `czz`.
...
// NodeInfo represents a short summary of the `czz` sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Classzz network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
...
// nodeInfo retrieves some `czz` protocol metadata about the running host node.
...
// Handle is invoked whenever an `czz` connection is made that successfully passes
// the protocol handshake. This method will keep processing messages until the
// connection is torn down.
func Handle(backend Backend, peer *Peer) error {
	for {
		if err := handleMessage(backend, peer); err != nil {
			peer.Log().Debug("Message handling failed in `czz`", "err", err)
			return err
		}
	}
}
