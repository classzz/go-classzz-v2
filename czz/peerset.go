...
var (
	...
	// snap protocol without advertizing the czz main protocol.
	errSnapWithoutEth = errors.New("peer connected on snap without compatible czz support")
)
...
type peerSet struct {
	...
	snapPend map[string]*snap.Peer      // Peers connected on the `snap` protocol, but not yet on `czz`
...
// registerSnapExtension unblocks an already connected `czz` peer waiting for its
// `snap` extension, or if no such peer exists, tracks the extension for the time
// being until the `czz` main protocol starts looking for it.
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	...
	// Inject the peer into an `czz` counterpart is available, otherwise save for later
...
// len returns if the current number of `czz` peers in the set. Since the `snap`
// peers are tied to the existence of an `czz` connection, that will always be a
// subset of `czz`.
