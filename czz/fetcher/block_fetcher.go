...
var (
	blockAnnounceInMeter   = metrics.NewRegisteredMeter("czz/fetcher/block/announces/in", nil)
	blockAnnounceOutTimer  = metrics.NewRegisteredTimer("czz/fetcher/block/announces/out", nil)
	blockAnnounceDropMeter = metrics.NewRegisteredMeter("czz/fetcher/block/announces/drop", nil)
	blockAnnounceDOSMeter  = metrics.NewRegisteredMeter("czz/fetcher/block/announces/dos", nil)

	blockBroadcastInMeter   = metrics.NewRegisteredMeter("czz/fetcher/block/broadcasts/in", nil)
	blockBroadcastOutTimer  = metrics.NewRegisteredTimer("czz/fetcher/block/broadcasts/out", nil)
	blockBroadcastDropMeter = metrics.NewRegisteredMeter("czz/fetcher/block/broadcasts/drop", nil)
	blockBroadcastDOSMeter  = metrics.NewRegisteredMeter("czz/fetcher/block/broadcasts/dos", nil)

	headerFetchMeter = metrics.NewRegisteredMeter("czz/fetcher/block/headers", nil)
	bodyFetchMeter   = metrics.NewRegisteredMeter("czz/fetcher/block/bodies", nil)

	headerFilterInMeter  = metrics.NewRegisteredMeter("czz/fetcher/block/filter/headers/in", nil)
	headerFilterOutMeter = metrics.NewRegisteredMeter("czz/fetcher/block/filter/headers/out", nil)
	bodyFilterInMeter    = metrics.NewRegisteredMeter("czz/fetcher/block/filter/bodies/in", nil)
	bodyFilterOutMeter   = metrics.NewRegisteredMeter("czz/fetcher/block/filter/bodies/out", nil)
)
...
type BlockFetcher struct {
	...
	fetchingHook       func([]common.Hash)               // Method to call upon starting a block (czz/61) or header (czz/62) fetch
	completingHook     func([]common.Hash)               // Method to call upon starting a block body fetch (czz/62)
	importedHook       func(*types.Header, *types.Block) // Method to call upon successful header or block import (both czz/61 and czz/62)
}
