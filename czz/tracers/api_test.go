...
func TestTraceCall(t *testing.T) {
	...
	var testSuite = []struct {
		...
		// Standard JSON trace upon the genesis, plain transfer.
		{
			...
			config:    nil,
			expectErr: nil,
			expect: &czzapi.ExecutionResult{
				Gas:         params.TxGas,
				Failed:      false,
				ReturnValue: "",
				StructLogs:  []czzapi.StructLogRes{},
			},
		},
	...
	var testSuite = []struct {
		...
		// Succcessful call with state overriding
		{
			...
			config: &TraceCallConfig{
				...
				StateOverrides: &czzapi.StateOverride{
					randomAccounts[0].addr: czzapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Ether)))},
				},
			},
			expectErr: nil,
			expect: &callTrace{
		...
		//  }
		{
			...
			config: &TraceCallConfig{
				Tracer: &tracer,
				StateOverrides: &czzapi.StateOverride{
					randomAccounts[2].addr: czzapi.OverrideAccount{
...
func TestTraceTransaction(t *testing.T) {
	...
	if !reflect.DeepEqual(result, &czzapi.ExecutionResult{
		Gas:         params.TxGas,
		Failed:      false,
		ReturnValue: "",
		StructLogs:  []czzapi.StructLogRes{},
