...
import (
	...
	"github.com/classzz/go-classzz-v2/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Gczz console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = czz.getBlock(czz.getTransaction(tx).blockHash);
  var genesis = czz.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.czz.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  czz.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
