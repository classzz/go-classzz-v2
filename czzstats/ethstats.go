...
import (
	...
	ethproto "github.com/classzz/go-classzz-v2/czz/protocols/czz"
...
// backend encompasses the bare-minimum functionality needed for czzstats reporting
...
// Service implements an Classzz netstats reporting daemon that pushes local
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(czzstats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	var network string
	if info := infos.Protocols["czz"]; info != nil {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the czzstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to czzstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to czzstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to czzstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to czzstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to czzstats")
