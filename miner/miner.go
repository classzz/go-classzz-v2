...
func New(czz Backend, config *Config, chainConfig *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, isLocalBlock func(block *types.Block) bool) *Miner {
	miner := &Miner{
		czz:     czz,
		mux:     mux,
		engine:  engine,
		...
		worker:  newWorker(config, chainConfig, engine, czz, mux, isLocalBlock, true),
	}
	go miner.update()
