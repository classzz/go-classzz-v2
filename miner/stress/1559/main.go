...
func makeMiner(genesis *core.Genesis) (*node.Node, *czz.Classzz, error) {
	// Define the basic configurations for the Classzz node
	...
	// Create the node and configure a full Classzz node on it
	stack, err := node.New(config)
	if err != nil {
		return nil, nil, err
	}
	ethBackend, err := czz.New(stack, &czzconfig.Config{
		Genesis:         genesis,
		...
		GPO:             czzconfig.Defaults.GPO,
		Ethash:          czzconfig.Defaults.Ethash,
		Miner: miner.Config{
