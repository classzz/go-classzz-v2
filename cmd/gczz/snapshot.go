...
var (
	snapshotCommand = cli.Command{
		...
		Subcommands: []cli.Command{
			{
				Name:      "prune-state",
				Usage:     "Prune stale classzz state data based on the snapshot",
				ArgsUsage: "<root>",
				...
				Description: `
gczz snapshot prune-state <state-root>
...
during the use of Gczz, please also specify it here for correct deletion. Otherwise
...
func pruneState(ctx *cli.Context) error {
	...
	pruner, err := pruner.NewPruner(chaindb, stack.ResolvePath(""), stack.ResolvePath(config.Czz.TrieCleanCacheJournal), ctx.GlobalUint64(utils.BloomFilterSizeFlag.Name))
	if err != nil {
