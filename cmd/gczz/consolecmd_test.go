...
const (
	ipcAPIs  = "admin:1.0 debug:1.0 czz:1.0 ethash:1.0 miner:1.0 net:1.0 personal:1.0 rpc:1.0 txpool:1.0 web3:1.0"
	httpAPIs = "czz:1.0 net:1.0 rpc:1.0 web3:1.0"
)
...
func TestConsoleWelcome(t *testing.T) {
	...
	gczz.SetTemplateFunc("apis", func() string { return ipcAPIs })
...
func TestAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\gczz` + strconv.Itoa(trulyRandInt(100000, 999999))
	} else {
		ws := tmpdir(t)
		defer os.RemoveAll(ws)
		ipc = filepath.Join(ws, "gczz.ipc")
	}
	...
	t.Run("ipc", func(t *testing.T) {
		...
		testAttachWelcome(t, gczz, "ipc:"+ipc, ipcAPIs)
	})
	t.Run("http", func(t *testing.T) {
		endpoint := "http://127.0.0.1:" + httpPort
		waitForEndpoint(t, endpoint, 3*time.Second)
		testAttachWelcome(t, gczz, endpoint, httpAPIs)
	})
...
func testAttachWelcome(t *testing.T, gczz *testgeth, endpoint, apis string) {
	...
	attach.SetTemplateFunc("datadir", func() string { return gczz.Datadir })
