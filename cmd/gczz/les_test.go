...
func (g *gethrpc) addPeer(peer *gethrpc) {
	...
	if err != nil {
		g.gczz.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gczz.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gczz.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gczz.Error("timeout adding peer after", dur)
	}
}
...
func (g *gethrpc) waitSynced() {
	...
	if ok && !syncing {
		g.gczz.Logf("%v already synced", g.name)
		return
	}

	// Actually wait, subscribe to the event
	ch := make(chan interface{})
	sub, err := g.rpc.Subscribe(context.Background(), "czz", ch, "syncing")
	if err != nil {
		g.gczz.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.gczz.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.gczz.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.gczz.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.gczz.Fatalf("%v timeout syncing", g.name)
		break
	}
}
