...
// PublicClasszzAPI provides an API to access Classzz related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicClasszzAPI struct {
	b Backend
}

// NewPublicClasszzAPI creates a new Classzz protocol API.
func NewPublicClasszzAPI(b Backend) *PublicClasszzAPI {
	return &PublicClasszzAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *PublicClasszzAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *PublicClasszzAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *PublicClasszzAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *PublicClasszzAPI) Syncing() (interface{}, error) {
...
// Sign calculates an Classzz ECDSA signature for:
...
//
// https://github.com/classzz/go-classzz-v2/wiki/Management-APIs#personal_sign
...
//
// https://github.com/classzz/go-classzz-v2/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Classzz signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gczz.
...
// PublicBlockChainAPI provides an API to access the Classzz blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Classzz blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current classzz chain config.
...
// See: https://github.com/classzz/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/classzz/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Classzz APIs exposed over the public
// debugging endpoint.
...
// Version returns the current classzz protocol version.
