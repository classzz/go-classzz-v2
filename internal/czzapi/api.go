...
// keccack256("\x18Classzz Signed Message:\n" + len(message) + message))
//
...
// the address of:
// hash = keccak256("\x18Classzz Signed Message:\n"${message length}${message})
...
func (api *PublicBlockChainAPI) ChainId() (*hexutil.Big, error) {
	...
	if config := api.b.ChainConfig(); config != nil {
...
func (s *PublicBlockChainAPI) GetLockBalance(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (*hexutil.Big, error) {
	...
	return (*hexutil.Big)(state.GetTeWakaStateLocked(address)), state.Error()
}
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f czz) exceeds the configured cap (%.2f czz)", feeFloat, cap)
	}
	return nil
}
