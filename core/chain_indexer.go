...
type ChainIndexer struct {
	chainDb  czzdb.Database      // Chain database to index the data from
	indexDb  czzdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb czzdb.Database, indexDb czzdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
