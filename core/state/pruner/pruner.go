...
func NewPruner(db czzdb.Database, datadir, trieCachePath string, bloomSize uint64) (*Pruner, error) {
...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb czzdb.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db czzdb.Database, trieCachePath string) error {
...
func extractGenesis(db czzdb.Database, stateBloom *stateBloom) error {
...
pruning. Remember don't start the Gczz without deleting the clean trie cache
otherwise the entire database may be damaged!

Check the command description "gczz snapshot prune-state --help" for more details.
`
