...
func loadAndParseJournal(db czzdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Gczz
	...
	// It can happen that Gczz crashes without persisting the latest
	// diff journal.
...
func loadSnapshot(diskdb czzdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, bool, error) {
	...
	//
	// Possible scenario: Gczz was crashed without persisting journal and then
