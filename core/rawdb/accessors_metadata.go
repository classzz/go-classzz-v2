...
func ReadDatabaseVersion(db czzdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db czzdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db czzdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db czzdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func PushUncleanShutdownMarker(db czzdb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db czzdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
