...
func ReadSnapshotDisabled(db czzdb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db czzdb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db czzdb.KeyValueWriter) {
...
func ReadSnapshotRoot(db czzdb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db czzdb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db czzdb.KeyValueWriter) {
...
func ReadAccountSnapshot(db czzdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db czzdb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db czzdb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db czzdb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db czzdb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db czzdb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db czzdb.Iteratee, accountHash common.Hash) czzdb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db czzdb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db czzdb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db czzdb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db czzdb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db czzdb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db czzdb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db czzdb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db czzdb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db czzdb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db czzdb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db czzdb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSnapshotSyncStatus(db czzdb.KeyValueWriter) {
