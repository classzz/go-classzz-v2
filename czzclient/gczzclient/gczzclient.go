...
// Package gczzclient provides an RPC client for gczz-specific APIs.
package gczzclient
...
// Client is a wrapper around rpc.Client that implements gczz-specific functionality.
//
// If you want to use the standardized Classzz RPC functionality, use czzclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg classzz.CallMsg) (*types.AccessList, uint64, string, error) {
...
// the message call.
// Please use czzclient.CallContract instead if you don't need the override functionality.
func (ec *Client) CallContract(ctx context.Context, msg classzz.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gczz node.
...
// MemStats retrieves the current memory stats from a gczz node.
...
// GetNodeInfo retrieves the node info of a gczz node.
