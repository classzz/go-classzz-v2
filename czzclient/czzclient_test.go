...
// Verify that Client implements the classzz interfaces.
var (
	_ = classzz.ChainReader(&Client{})
	_ = classzz.TransactionReader(&Client{})
	_ = classzz.ChainStateReader(&Client{})
	_ = classzz.ChainSyncReader(&Client{})
	_ = classzz.ContractCaller(&Client{})
	_ = classzz.GasEstimator(&Client{})
	_ = classzz.GasPricer(&Client{})
	_ = classzz.LogFilterer(&Client{})
	_ = classzz.PendingStateReader(&Client{})
	// _ = classzz.PendingStateEventer(&Client{})
	_ = classzz.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Classzz Service
	config := &czzconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := czz.New(n, config)
	if err != nil {
		t.Fatalf("can't create new classzz service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == classzz.NotFound {
		t.Fatal("error should not be nil/notfound")
	}
	// Test tx in block not found
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 1); err != classzz.NotFound {
		t.Fatal("error should be classzz.NotFound")
	}
}
