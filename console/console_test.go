...
func newTester(t *testing.T, confOverride func(*czzconfig.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Classzz service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	ethConf := &czzconfig.Config{
	...
	ethBackend, err := czz.New(stack, ethConf)
	if err != nil {
		t.Fatalf("failed to register Classzz protocol: %v", err)
	}
