...
import (
	...
	"github.com/classzz/go-classzz-v2/internal/jsre/deps"
	"github.com/classzz/go-classzz-v2/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client         // RPC client to execute Classzz requests through
...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"czz": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines czz.getBalance(czz.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gczz instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	message := "Welcome to the Gczz JavaScript console!\n\n"

	// Print some generic Gczz metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + czz.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + czz.blockNumber + " (" + new Date(1000 * czz.getBlock(czz.blockNumber).timestamp) + ")\n";
		try {
