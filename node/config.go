...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gczz is "gczz". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gczz", keep that.
	if name == "gczz" || name == "gczz-testnet" {
		name = "Gczz"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gczz" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gczz 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'gczz' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-classzz-v2-keystore")
		ephemeral = keydir
	}
