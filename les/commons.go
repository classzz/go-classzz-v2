...
// NodeInfo represents a short summary of the Classzz sub-protocol metadata
...
func (c *lesCommons) setupOracle(node *node.Node, genesis common.Hash, czzconfig *czzconfig.Config) *checkpointoracle.CheckpointOracle {
	config := czzconfig.CheckpointOracle
	if config == nil {
	...
	client := czzclient.NewClient(rpcClient)
	oracle.Start(client)
