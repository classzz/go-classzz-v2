...
import (
	...
	"github.com/classzz/go-classzz-v2/les/vflux"
...
func New(stack *node.Node, config *czzconfig.Config) (*lightClasszz, error) {
	...
	lesDb, err := stack.OpenDatabase("les.client", 0, 0, "czz/db/lesclient/", false)
	if err != nil {
		return nil, err
	}
	...
	leth := &lightClasszz{
		...
		engine:         czzconfig.CreateConsensusEngine(stack, chainConfig, &config.Ethash, nil, false, chainDb),
	...
	leth.netRPCService = czzapi.NewPublicNetAPI(leth.p2pServer, leth.config.NetworkId)
...
func (s *lightClasszz) Stop() error {
	...
	s.lesDb.Close()
	s.wg.Wait()
	log.Info("Light classzz stopped")
	return nil
}
