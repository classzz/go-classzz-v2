...
// the czz.BlockFetcher as the underlying fetcher but adding more additional
...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb czzdb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
...
//
// Note, we rely on the underlying czz/fetcher to retrieve and validate the
// response, so that we have to obey the rule of czz/fetcher which only accepts
