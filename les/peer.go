...
import (
	...
	"github.com/classzz/go-classzz-v2/les/utils"
	vfc "github.com/classzz/go-classzz-v2/les/vflux/client"
...
// PeerInfo represents a short summary of the `czz` sub-protocol metadata known
// about a connected peer.
type PeerInfo struct {
	Version    int      `json:"version"`    // Classzz protocol version negotiated
...
func (p *peerCommons) handshake(td *big.Int, head common.Hash, headNum uint64, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter, sendCallback func(*keyValueList), recvCallback func(keyValueMap) error) error {
	...
	// as well. Check http://eips.classzz.org/EIPS/eip-2124 for more
	// spec detail.
	if p.version >= lpv4 {
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local classzz node is running in archive mode, advertise ourselves we have
...
// participating in the Light Classzz sub-protocol.
...
// for us(server) but for other protocols(e.g. CZZ) upon the devp2p they
// may be useful.
