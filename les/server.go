...
type LesServer struct {
	...
	archiveMode bool // Flag whether the classzz node runs in archive mode.
...
func NewLesServer(node *node.Node, e ethBackend, config *czzconfig.Config) (*LesServer, error) {
	lesDb, err := node.OpenDatabase("les.server", 0, 0, "czz/db/lesserver/", false)
	if err != nil {
		return nil, err
	}
	...
	srv.vfluxServer.Register(srv.clientPool, "les", "Classzz light client service")
