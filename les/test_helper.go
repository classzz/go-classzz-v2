...
import (
	...
	"github.com/classzz/go-classzz-v2/accounts/abi/bind/backends"
	...
	"github.com/classzz/go-classzz-v2/contracts/checkpointoracle/contract"
	...
	"github.com/classzz/go-classzz-v2/les/checkpointoracle"
	"github.com/classzz/go-classzz-v2/les/flowcontrol"
	vfs "github.com/classzz/go-classzz-v2/les/vflux/server"
...
func prepare(n int, backend *backends.SimulatedBackend) {
	...
	for i := 0; i < n; i++ {
		...
		case 0:
			...
			// bankUser transfers some czz to user1
		...
		case 1:
			...
			// bankUser transfers more czz to user1
		...
		case 2:
			...
			// bankUser transfer some czz to signer
			bankNonce, _ := backend.PendingNonceAt(ctx, bankAddr)
...
func testIndexers(db czzdb.Database, odr light.OdrBackend, config *light.IndexerConfig, disablePruning bool) []*core.ChainIndexer {
...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db czzdb.Database, peers *serverPeerSet, ulcServers []string, ulcFraction int) (*clientHandler, func()) {
	var (
	...
	client := &LightClasszz{
		lesCommons: lesCommons{
			...
			config:      &czzconfig.Config{LightPeers: 100, NetworkId: NetworkId},
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db czzdb.Database, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend, func()) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
